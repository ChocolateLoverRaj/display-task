...
Promise.all(dirs.map(async dir => {
  ...
  const { code } = await generator.default(
    types.addComment(types.program([
      types.exportNamedDeclaration(
        undefined,
        [types.exportSpecifier(types.identifier('default'), types.identifier('Addon'))],
        types.stringLiteral('./addon.js')
      )
    ]), 'leading', ' Generated by build/build.js', true)
  )
